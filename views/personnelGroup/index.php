...
<script>
 jQuery('#groupFetch').load('fetch.php', 'f' + (Math.random()*100000));
</script>
...
<script>
  ...
  $.ajax({
      ...
      success: function(groupCheckData){
          if(groupCheckData == 1)
          {
...
function openUpdateModal(id,name)
{
  ...
  document.getElementById("originalGroupName").value=name;

}

$(document).on("click", "#updateGroup", function() { 

  if($('#updateGroupName').val() == $('#originalGroupName').val())
  {
